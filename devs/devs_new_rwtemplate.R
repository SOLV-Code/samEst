...
TMB::compile('Ricker_tv_all.cpp')
dyn.load(dynlib('src/Ricker_tv_all'))

#data
exdat<-list(priors_flag=1,
    stan_flag=0,
    sig_p_sd=1,
    siga_p_sd=1,
    sigb_p_sd=1,
    ...
    logb_p_sd=3)
exdat$obs_S = harck$S
exdat$obs_logRS = harck$logRS
exdat$options_z<-c(1,0)

initlm<-lm(exdat$obs_logRS~exdat$obs_S)


#params
tvallparams <- list(logalpha = 1,
                 logbeta = log(-initlm$coefficients[[2]]),
                 logsigobs = 0,
                 logsiga = 0,
                 logsigb = 0,
                 
                 epslogalpha_t = rep(0,nrow(harck)),
                 epslogbeta_t = rep(0,nrow(harck)) )
Map <- list()
Map$logsigb = factor(NA)
Map$epslogbeta_t = factor( rep(NA,length(tvallparams$epslogbeta_t)) )


objex <- TMB::MakeADFun(data = exdat, 
      parameters = tvallparams, 
      map = Map,
      random = c( "epslogalpha_t"), 
      DLL = "Ricker_tv_all")
...
nonvariance_fixed_effects = c("logalpha","logbeta")
  ...
  # Optimize first time
optex <- nlminb( start=objex$par, obj=objex$fn, grad=objex$gr )
sd_report_ex <- TMB::sdreport(objex)
convex <- get_convergence_diagnostics(sd_report_ex)



ptva<- ricker_rw_TMB(data=harck,tv.par="a",sig_p_sd=1,priors_flag=1, deltaEDF=0.0001)
ptva$conv_problem
ptva$model$convergence

objex$report()$logalpha
plot(objex$report()$logalpha_t, type="b")
lines(ptva$alpha, type="b", col="darkred")
...
exdat_b<-list(priors_flag=1,
    ...
    logb_p_mean=-12,
    logb_p_sd=3)
exdat_b$obs_S = harck$S
exdat_b$obs_logRS = harck$logRS
exdat_b$options_z<-c(0,1)



#params
Map_b <- list()
Map_b$logsiga = factor(NA)
Map_b$epslogalpha_t = factor( rep(NA,length(tvallparams$epslogalpha_t)) )


objex_b <- TMB::MakeADFun(data = exdat_b, 
      parameters = tvallparams, 
      map = Map_b,
      random = c( "epslogbeta_t"), 
      DLL = "Ricker_tv_all")



  # Optimize first time
optex_b <- nlminb( start=objex_b$par, obj=objex_b$fn, grad=objex_b$gr )
ptvb<- ricker_rw_TMB(data=harck,tv.par="b",sig_p_sd=1,priors_flag=1)

ptvb$beta
objex_b$report()$logbeta
plot(objex_b$report()$Smax_t, type="b")
lines(ptvb$Smax, type="b", col="darkred")
...
exdat_ab<-list(priors_flag=1,
    ...
    logb_p_mean=-12,
    logb_p_sd=3)
exdat_ab$obs_S = harck$S
exdat_ab$obs_logRS = harck$logRS
exdat_ab$options_z<-c(1,1)



#params

objex_ab <- TMB::MakeADFun(data = exdat_ab, 
      parameters = tvallparams, 
      random = c("epslogalpha_t", "epslogbeta_t"), 
      DLL = "Ricker_tv_all")



  # Optimize first time
optex_ab <- nlminb( start=objex_ab$par, obj=objex_ab$fn, grad=objex_ab$gr )
ptvab<- ricker_rw_TMB(data=harck,tv.par="both",sig_p_sd=1,priors_flag=0)
...
objex_b$report()$pnll
ptvb$tmb_obj$report()$pnll

objex_b$report()$renll
ptvb$tmb_obj$report()$renll

objex_b$report()$nll
ptvb$tmb_obj$report()$nll

par(mfrow=c(1,2))
plot(objex_ab$report()$Smax_t, type="b", ylim=range(c(objex_ab$report()$Smax_t,ptvab$Smax)))
lines(ptvab$Smax, type="b", col="darkred")
plot(objex_ab$report()$logalpha_t, type="b")
lines(ptvab$alpha, type="b", col="darkred")


ptvab$model$convergence
optex_ab$convergence
...
#b
EDF_b = calculate_EDF(obj=objex_b,
                      opt=optex_b,
                      nonvariance_fixed_effects=nonvariance_fixed_effects,
                      refit = "full",
                      delta=0.0001)
